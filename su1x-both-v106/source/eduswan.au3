...
;
;
; Updated 13/09/10 - Gareth Ayres (g.j.ayres@swan.ac.uk)
	...
	;Set the IE proxy settings to Automaticaly Detect
            ...
            If @error = 0 Then
                        ...
                        If (RegWrite("HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Internet Settings\Connections","WiFiConfigBackup", "REG_BINARY",$key_data)==1) Then
									...
									DoDebug("Backed up IE Reg key to WiFiConfigBackup and added new one")
                        EndIf
            EndIf
...
doDebug("_Wlan_SetProfileUserData"&$hClientHandle&$pGUID&$SSID&$credentials)
$setCredentials = _Wlan_SetProfileUserData($hClientHandle, $pGUID, $SSID, $credentials)
If @error Then 
	DoDebug("credential error:" & @ScriptLineNumber & @error & @extended & $setCredentials)
EndIf
DoDebug("Set Credentials=" &$credentials[2] & $credentials[3]& $setCredentials)
...
DoDebug("send_ldap=" & $send_ldap) 
	...
	DoDebug("pass="&$pass)
	DoDebug("https://swisweb.swan.ac.uk/logintest.php?email="& $user & "&" & "pass=" & $pass)
